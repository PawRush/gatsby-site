...
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `MarcySuttonFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
if (codeConnectionArn) {
  new PipelineStack(app, "MarcySuttonComPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for MarcySuttonCom",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "MarcySutton");
