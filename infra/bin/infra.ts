...
// Manual deployments (when codeConnectionArn is not provided)
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `MarcyFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
if (codeConnectionArn) {
  new PipelineStack(app, "MarcyPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for Marcy",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "Marcy");
