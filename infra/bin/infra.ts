...
// Manual deployment stacks (used when no pipeline context provided)
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `MarcySiteFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
// Pipeline stack (used when pipeline context provided)
if (codeConnectionArn) {
  new PipelineStack(app, "MarcySitePipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for MarcySite",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "MarcySite");
