...
export class FrontendStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: FrontendStackProps) {
    ...
    // URL rewrite function for Gatsby (trailing slash: always)
    ...
    // Add URL rewrite function to distribution
    const distribution = cloudfrontToS3.cloudFrontWebDistribution;
    const websiteBucket = cloudfrontToS3.s3Bucket!;

    // Update default behavior to include URL rewrite function
    const cfnDistribution = distribution.node.defaultChild as cloudfront.CfnDistribution;
    cfnDistribution.distributionConfig = {
      ...cfnDistribution.distributionConfig,
      defaultCacheBehavior: {
        ...(cfnDistribution.distributionConfig as any).defaultCacheBehavior,
        functionAssociations: [
          {
